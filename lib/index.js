...
export default function slackin({
  ...
  app.get('/', (req, res) => {
    ...
    let page = dom('html',
      dom('head',
        dom('title',
          'Junte-se ao PHPSP no Slack!'
          // 'Junte-se ao ', name, ' no Slack!'
        ),
  ...
  app.post('/invite', json(), (req, res, next) => {
    ...
    if (coc && '1' != req.body.coc) {
      return res
      .status(400)
      .json({ msg: 'É obrigatório concordar com o CoC' });
    }
    ...
    invite({ token, org, email, channel: chanId }, err => {
      ...
      res
      .status(200)
      .json({ msg: 'Tudo certo! Verifique seu e-mail!' });
    });
  });
  ...
  app.get('/iframe/dialog', (req, res) => {
    ...
    let { active, total } = slack.users;
    if (!name) return res.send(404);
    let dom = splash({ org, coc, path, name, channels, active, total, iframe: true });
    res.type('html');
    res.send(dom.toHTML());
  });
