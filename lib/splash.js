...
export default function splash({ name, logo, active, total, channels, iframe }){
  let div = dom('.splash',
    ...
    dom('p',
      // 'Junte-se ao ', dom('b', name),
      'Junte-se ao ', dom('b', 'PHPSP'),
    ...
    dom('p.status',
      active
        ...
        : [dom('b.total', total), ' usuários registrados até agora.']
    ),
    dom('form',
      ...
      dom('input.form-item type=email placeholder=seu@email.com.br '
        + (!iframe ? 'autofocus' : '')),
      dom('button.loading', 'Receber meu convite!')
    ),
    dom('a class=button href=https://phpsp.slack.com/ target=_blank',
      'Já está cadastrado?'
    ),
    !iframe && dom('footer',
      'powered by ',
